...
export const chart: TemplateChartFn = async (construct, _, context) => {
  ...
  const musicAssistant = svc(
    "music-assistant",
    pod("music-assistant", "music-assistant"),
  );
  ...
  homeAssistant
    ...
    .to(gatewayFamily, tcp(443))
    .to(llamaCppServer, tcp(8080))
    .to(mosquitto, tcp(1883))
    .to(musicAssistant, tcp(8095))
  ...
  musicAssistant
    ...
    .to(cidrs("224.0.0.251/32"), udp(5353))
    .to(cidrs("192.168.24.0/24"))
    .to(gatewayFamily, tcp(443), tcp(8097))
  ...
  gatewayFamily
    ...
    .from(
      cidrs(
        ...
        "192.168.24.2/32",
        // iot + iot_allow_intranet (sonos (play right))
        "192.168.24.3/32",
        // iot + iot_allow_intranet (sonos (play left))
        "192.168.24.4/32",
        // iot + iot_allow_intranet (sonos (bedroom-2 right))
        "192.168.24.6/32",
        // iot + iot_allow_intranet (sonos (bedroom-2 left))
        "192.168.24.7/32",
