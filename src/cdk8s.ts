...
interface HostEndpointOpts {
  profiles?: string[];
}

export class HostEndpoint extends ApiObject {
  constructor(
    construct: Construct,
    hostname: string,
    interfaceSelector: string,
    opts: HostEndpointOpts = {},
  ) {
    const interfaceName = interfaceSelector === "*" ? "all" : interfaceSelector;
    const id = `${construct.node.id}-host-endpoint-${hostname}-${interfaceName}`;
    const profiles = opts.profiles ?? ["projectcalico-default-allow"];
    const labels: Record<string, string> = {
      "kubernetes.io/hostname": hostname,
    };
    if (interfaceSelector !== "*") {
      labels["homelab/interface"] = interfaceSelector;
    }
    ...
    super(construct, id, {
      ...
      metadata: {
        name: `${hostname}-${interfaceName}`,
        labels,
      },
      spec: {
        interfaceName: interfaceSelector,
        node: hostname,
        profiles,
      },
    });
...
export class Tier extends ApiObject {
  constructor(
    construct: Construct,
    name: string,
    order: number,
    ...
    const defaultAction = opts.action ?? "Pass";
    const id = `${construct.node.id}-tier-${name}`;
